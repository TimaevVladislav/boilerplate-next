...
describe("Campaigns", () => {
    ...
    it ("allows people to contribute money and marks them as approvers", async () => {
        const account = await campaign.methods.contribute()
              .send({ value: "200", from: accounts[1] })

        const isContributor = await campaign.methods.approvers(account)
    ...
    it ("processes requests", async () => {
        ...
        await campaign.methods.approveRequest(0).send({ from: accounts[0], gas: "100000" })
        await campaign.methods.finalizeRequest(0).send({ from: accounts[0], gas: "100000" })
